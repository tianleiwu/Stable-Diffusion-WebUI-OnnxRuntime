...
import torch
from modules import devices, paths_internal, script_callbacks, sd_unet, shared
...
class OrtUnetOption(sd_unet.SdUnetOption):
    ...
    def create_unet(self):
        if shared.sd_model.is_sdxl:
            raise ValueError(
                "SD XL models are not supported with this version of OnnxRuntime extension."
            )
...
class CudaSession:
    ...
    def allocate_buffers(self, shape_dict: Dict[str, Union[Tuple[int], List[int]]]):
        ...
        for name, shape in shape_dict.items():
            if name in self.output_names:
                # Reuse allocated buffer when the shape is same
                if name in self.output_tensors and tuple(
                    self.output_tensors[name].shape
                ) == tuple(shape):
                    continue

                numpy_dtype = self.io_name_to_numpy_type[name]
                tensor = torch.empty(
                    tuple(shape), dtype=TypeHelper.numpy_type_to_torch_type(numpy_dtype)
                ).to(device=self.device)
    ...
    def get_cuda_provider_options(
        device_id: int, enable_cuda_graph: bool
    ) -> Dict[str, Any]:
        return {
            "device_id": device_id,
...
class OrtUnet(sd_unet.SdUnet):
    def __init__(self, filename, *args, **kwargs):
        ...
        self.filename = filename
        self.is_turbo = "turbo" in os.path.basename(filename).lower()
        self.session = None
    ...
    def is_input_shape_changed(self, feed_dict):
        if (
            (self.dimensions["width"] != feed_dict["x"].shape[-1])
            or (self.dimensions["height"] != feed_dict["x"].shape[-2])
            or (self.dimensions["batch_size"] != feed_dict["x"].shape[-4])
        ):
            gr.Info(
                "Dimensions changed, restarting ORT inference session, first job may be slow. \
                Disable static dimensions from Settings if you expect to change image size/batch size frequently"
            )
            return True
    ...
    def infer(self, feed_dict):
        ...
        if (
            ...
            or new_profile != self.static_dimension_dims
            or (self.static_dimension_dims and self.is_input_shape_changed(feed_dict))
        ):
            self.dimensions["width"] = feed_dict["x"].shape[-1]
    ...
    def forward(self, x, timesteps, context, *args, **kwargs):
        remove_uncond = self.is_turbo and shared.opts.data.get(
            "ort_turbo_cfg1_opt", False
        )
        if remove_uncond:
            # For turbo model, there is no need for classifier free guidance.
            # Here is a walkaround to remove uncond data that appended even when CFG=1.0.
            batch_size = x.shape[0]
            if batch_size % 2 == 0 and self.is_turbo:
        ...
        out = outputs["out_sample"]

        # restore uncond
        if remove_uncond:
            out = torch.cat([out, out])

        return out.to(dtype=x.dtype, device=devices.device)
    ...
    def create_session(self):
        ...
        session = ort.InferenceSession(
            self.filename, providers=providers, sess_options=self.sess_options
        )
...
def list_unets(unet_list):
    ort_dir = os.path.join(paths_internal.models_path, "UNET-ORT")
    candidates = list(shared.walk_files(ort_dir, allowed_extensions=[".onnx"]))
...
def on_ui_settings():
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            False,
            "Enable ORT Unet Static Dimensions",
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ),
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            False,
            "Enable Turbo CFG 1.0 Optimization",
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ),
...
def on_ui_tabs():
    with gr.Blocks() as ort_interface:
