...
class UnetAttentionFusion(Fusion):
    """
    ...
    def __init__(
        ...
        weight_packing_list: Optional[List] = None,
    ):
        super().__init__(
            model,
        ...
        # A list to trace weight packing
    ...
    def get_hidden_size(self, layer_norm_node):
        """Detect hidden_size from LayerNormalization node.
        Args:
            layer_norm_node (NodeProto): LayerNormalization node before Q, K and V
        Returns:
            int: hidden_size, or 0 if not found
        """
        layer_norm_bias = self.model.get_initializer(layer_norm_node.input[2])
        if layer_norm_bias:
            return NumpyHelper.to_array(layer_norm_bias).shape[0]
    ...
    def create_attention_node(
        ...
        weight_packing = {
            "source": [
                {
                    "name": q_matmul.input[1],
                    "hash": hash_as_fp16_numpy_array(qw),
                    "shape": list(qw.shape),
                    "alias": "qw",
                },
                {
                    "name": k_matmul.input[1],
                    "hash": hash_as_fp16_numpy_array(kw),
                    "shape": list(kw.shape),
                    "alias": "kw",
                },
                {
                    "name": v_matmul.input[1],
                    "hash": hash_as_fp16_numpy_array(vw),
                    "shape": list(vw.shape),
                    "alias": "vw",
                },
            ],
        ...
        if is_self_attention:
            ...
            if self.enable_packed_qkv:
                ...
                weight_packing["target"] = {
                    "name": matmul_node_name + "_weight",
                    "shape": [qkv_weight.shape[0], qkv_weight.shape[1]],
                    "format": "qkv_mha",
                    "source_reshape": [c, n, h],
                    "transform": "np.dstack([qw, kw, vw)])",
                }
                ...
                weight_packing["target"] = {
                    ...
                    "shape": [qw_in_size, qkv_weight_dim],
                    "format": "qkv_att",
                    "transform": "np.stack((qw, kw, vw), axis=1)",
                }
            ...
            if self.enable_packed_kv:
                ...
                weight_packing["source"] = weight_packing["source"][1:]
                weight_packing["target"] = {
                    "name": matmul_node_name + "_weight",
                    "shape": list(kv_weight.shape),
                    "format": "kv_mha",
                    "source_reshape": [c, n, h],
                    "transform": "np.dstack([kw, vw)])",
                }
        ...
        if weight_packing and self.weight_packing_list is not None:
    ...
    def fuse(self, normalize_node, input_name_to_nodes, output_name_to_node):
        ...
        match_qkv = self.match_qkv(root_input, skip_add)
    ...
    def match_qkv(self, root_input, skip_add):
        """Match Q, K and V paths"""
