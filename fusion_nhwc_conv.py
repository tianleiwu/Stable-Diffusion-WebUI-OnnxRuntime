...
# This file is modified from https://github.com/microsoft/onnxruntime/blob/main/onnxruntime/python/tools/transformers/fusion_nhwc_conv.py
...
class FusionNhwcConv(Fusion):
    ...
    def fuse(self, conv, input_name_to_nodes, output_name_to_node):
        ...
        if not (dtype is not None and weight_tensor.data_type == dtype):
            nhwc_conv_input, _cast_node = self.fusion_utils.cast_input(
                nhwc_conv_input,
                "float16" if weight_tensor.data_type == TensorProto.FLOAT16 else "float32",
            )
